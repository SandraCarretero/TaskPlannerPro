...
function setupEventListeners() {
  ...
  document.getElementById("close-change-avatar").addEventListener("click", () => {
    changeAvatarModal.classList.add("hidden")
  })

  document.getElementById("cancel-change-avatar").addEventListener("click", () => {
    changeAvatarModal.classList.add("hidden")
  })

  // Previsualizar avatar
  document.getElementById("avatar-file").addEventListener("change", previewAvatar)
...
async function updateProfile(e) {
  ...
  try {
    ...
    document.getElementById("error-name").textContent = "Error al actualizar perfil"
  }
}
...
async function changePassword(e) {
  ...
  try {
    ...
    if (newPassword !== confirmNewPassword) {
      document.getElementById("error-confirm-new-password").textContent = "Las contraseñas no coinciden"
      return
    }
    ...
    if (error.message === "Contraseña actual incorrecta") {
      document.getElementById("error-current-password").textContent = "Contraseña actual incorrecta"
    } else {
      document.getElementById("error-current-password").textContent = "Error al cambiar contraseña"
    }
  }
}
...
function previewAvatar(e) {
  ...
  if (!file.type.startsWith("image/")) {
    document.getElementById("error-avatar-file").textContent = "El archivo debe ser una imagen"
    return
  }

  // Validar tamaño (máximo 2MB)
  if (file.size > 2 * 1024 * 1024) {
    document.getElementById("error-avatar-file").textContent = "La imagen no debe superar los 2MB"
    return
  }
...
async function changeAvatar(e) {
  ...
  try {
    ...
    if (!fileInput.files || !fileInput.files[0]) {
      document.getElementById("error-avatar-file").textContent = "Debe seleccionar una imagen"
      return
    }
    ...
    if (file.size > 5 * 1024 * 1024) {
      document.getElementById("error-avatar-file").textContent = "La imagen no debe superar los 5MB";
      return;
    }
    ...
    document.getElementById("error-avatar-file").textContent = "Error al cambiar avatar"
  }
}
